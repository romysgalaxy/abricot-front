    ...
    <div className={styles['modal-overlay']} onClick={onClose}>
      <div className={styles['modal']} onClick={(e) => e.stopPropagation()}>
        <div className={styles['modal-header']}>
          <h2 className={styles['modal-title']}>{isEditMode ? 'Modifier le projet' : 'Créer un projet'}</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={styles['modal-field']}>
            <label className={styles['modal-label']}>Titre *</label>
            <input
              type="text"
          ...
          <div className={styles['modal-field']} ref={dropdownRef}>
            <label className={styles['modal-label']}>Collaborateurs</label>
            <div className={styles['contributors-select']}>
              <div className={styles['contributors-input-wrapper']}>
                {selectedContributors.map((c) => {
                  const isOwner = c.id === currentUser?.id;
                  return (
                    <span key={c.id} className={`${styles['contributor-tag']}${isOwner ? ` ${styles['contributor-tag--owner']}` : ''}`}>
                      <span className={styles['contributor-tag-avatar']}>{getInitials(c)}</span>
                      <span className={styles['contributor-tag-name']}>{c.name || c.email}</span>
                      {!isOwner && (
                        <button
                          type="button"
                          className={styles['contributor-tag-remove']}
                ...
                <input
                  ref={inputRef}
                  type="text"
                  className={styles['contributors-search-input']}
              ...
              <button
                type="button"
                className={styles['contributors-toggle-btn']}
                onClick={handleToggleDropdown}
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className={showDropdown ? styles['arrow-rotated'] : ''}>
          ...
          <div className={styles['modal-actions']}>
            <button type="submit" className={styles['btn-submit']} disabled={loading || !name.trim() || !description.trim()}>
